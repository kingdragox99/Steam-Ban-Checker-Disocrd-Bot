...
async function crawlSteamProfile(startUrl = null) {
  ...
  // Ajouter tous les contacts à la base de données avant de commencer à crawler leurs contacts
  for (const contactUrl of contacts) {
    await addContact(contactUrl); // Ajouter les contacts
  }

  // Crawler les contacts après que tous aient été ajoutés
  for (const contactUrl of contacts) {
    await crawlSteamProfile(contactUrl); // Crawler récursivement
  }
